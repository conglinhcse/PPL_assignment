            ...
            if len(ast.body.member)==0:
                ...
                for member in ast.body.member:
                    if isinstance(member,VarDecl):
                        ...
                        ref_envi=[local_envi,c]
                        if isinstance(member,(Block,Dowhile,If,Return)):
                            check_return_in_body= self.visit(member,[ref_envi,False,returnType])
                        else:
                            self.visit(member,[ref_envi,False,returnType])
                    if check_return_in_body==True:
                        func_return=True
                if func_return==False:
                    raise FunctionNotReturn(ast.name.name)
        ...
        if len(ast.member)==0:
            ...
            for x in ast.member:
                if isinstance(x,VarDecl):
                    ...
                    if isinstance(x,(Return,Block,Dowhile,If)):
    ...
    def visitDowhile(self,ast,c):
        ...
        if len(ast.sl)==0:
            ...
            if isinstance(ast.sl[0],(Block,Return,If,Dowhile)):
                trigger= self.visit(ast.sl[0],[c[0],True,c[2]])
            else:
                if isinstance(ast.sl[0],(For,Break,Continue)):
                    self.visit(ast.sl[0],[c[0],True,c[2]])
            ...
            for x in ast.sl:
                if isinstance(x,VarDecl):
                    ...
                    ref_block=[[block_envi]+c[0],c[1],c[2]]
                    if isinstance(x,(Block,Dowhile,If,Return)):
                        trigger_child= self.visit(x,[[block_envi]+c[0],True,c[2]])
                    else:
                        self.visit(x,[[block_envi]+c[0],True,c[2]])
                else:
                    self.visit(x,[block_envi]+c[0])
